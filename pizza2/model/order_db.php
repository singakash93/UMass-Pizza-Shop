...
function order_count($db, $current_day){
    $query = 'select count(id) as order_count from pizza_orders where day = :day;';
    $statement = $db->prepare($query);
    $statement->bindValue(':day', $current_day);
    $statement->execute();
    $id = $statement->fetch()['order_count'];
    $statement->closeCursor();
    return (int)$id;
}